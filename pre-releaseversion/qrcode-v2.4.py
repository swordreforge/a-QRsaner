...
class QRScannerApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.binary_editor_module = BinaryEditorModule(self)  # 新增二进制编辑器模块
        ...
        # 新增二进制编辑器选项卡
        binary_editor_tab = ttk.Frame(self.notebook)
        self.notebook.add(binary_editor_tab, text="二进制编辑器")
        ...
        self.binary_editor_module.init_ui(binary_editor_tab)  # 初始化二进制编辑器UI
    ...
    def handle_drop(self, event):
        ...
        if current_tab == 0:  # 扫描选项卡
            ...
            valid_files = [f for f in files if f.lower().endswith(
                ('.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.gif', '.dat', '.bin','.webp'))]
            if valid_files:
        ...
        elif current_tab == 3:  # 二进制编辑器选项卡
            valid_files = [f for f in files if f.lower().endswith(('.dat', '.bin', '.hex', '.txt'))]
            if valid_files:
                self.binary_editor_module.handle_drop(valid_files)
...
class BinaryEditorModule:
    """二进制编辑器模块，支持多种编码格式和文件头尾标注"""
    def __init__(self, parent):
        self.parent = parent
        self.config = parent.config["binary_editor"]
        self.current_file = None
        self.binary_data = None
        self.encoding = self.config["encoding"]
        self.mode = "hex"  # 显示模式: hex或binary
        self.selected_start = None
        self.selected_end = None
        self.is_loading = False
        self.cancel_loading = False
        self.focused_widget = None  # 跟踪当前聚焦的文本框
        self.focus_indicator = None  # 状态指示器标签
        
        # 文件头/文件尾识别字典
        self.file_header_footer_dict = {
            b'\xFF\xD8\xFF': "JPEG文件头",
            b'\x89PNG\r\n\x1a\n': "PN极狐G文件头",
            b'\x50\x4B\x03\x04': "ZIP文件头",
            b'\x25\x50\x44\x46': "PDF文件头",
            b'\x49\x44\x33': "MP3文件头",
            b'\xFF\xD9': "JPEG文件尾",
            b'\x00\x00\x00\x00': "可能的填充区域"
        }
    def on_right_configure(self, event):
        """更新右侧编码显示区域的行号"""
        self.right_line_numbers.configure(state='normal')
        self.right_line_numbers.delete(1.0, tk.END)
        self.right_line_numbers.insert(tk.END, "\n".join(str(i) for i in range(1, self.right_text.count("1.0", tk.END, "lines"))))
        self.right_line_numbers.configure(state='disabled')

    def update_right_line_numbers(self, event=None):
        """动态更新右侧编码显示区域的行号"""
        self.right_line_numbers.configure(state='normal')
        self.right_line_numbers.delete(1.0, tk.END)
        line_numbers = self.right_text.count("1.0", tk.END, "lines")
        self.right_line_numbers.insert(tk.END, "\n".join(str(i) for i in range(1, int(line_numbers)+1)))
        self.right_line_numbers.configure(state='disabled')
    def init_ui(self, parent_frame):
        """初始化二进制编辑器选项卡UI"""
        # 主容器
        ...
        split_frame.add(right_frame, weight=1)

        right_header = ttk.Label(right_frame, text="编码值", font=("TkDefaultFont", 10, "bold"))
        right_header.pack(fill=tk.X, padx=5, pady=5)

        # 添加行号显示区域
        self.right_line_numbers = tk.Text(right_frame, wrap=tk.NONE, width=4, height=25, state='disabled', background='lightgrey')
        self.right_line_numbers.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)
        self.right_line_numbers.bind("<Configure>", self.on_right_configure)

        self.right_text = tk.Text(right_frame, wrap=tk.NONE, width=40, height=25)
        ...
        # 更新右侧行号显示
        self.right_text.bind("<KeyRelease>", self.update_right_line_numbers)
        # 顶部控制栏
        control_frame = ttk.Frame(editor_container)
        control_frame.pack(fill=tk.X, padx=10, pady=10)
        
        # 文件操作按钮
        self.open_button = ttk.Button(control_frame, text="打开文件", command=self.open_file)
        self.open_button.pack(side=tk.LEFT, padx=5)
        
        self.save_button = ttk.Button(control_frame, text="保存文件", command=self.save_file, state=tk.DISABLED)
        self.save_button.pack(side=tk.LEFT, pad极狐x=5)
        
        # 状态指示器
        status_indicator_frame = ttk.Frame(control_frame)
        status_indicator_frame.pack(side=tk.RIGHT, padx=10)
        
        self.focus_indicator = ttk.Label(status_indicator_frame, text="当前聚焦: -", font=("TkDefaultFont", 9))
        self.focus_indicator.pack(side=tk.RIGHT)
        
        # 显示模式切换
        mode_frame = ttk.Frame(control_frame)
        mode_frame.pack(side=tk.LEFT, padx=10)
        
        ttk.Label(mode_frame, text="显示模式:").pack(side=tk.LEFT)
        
        self.mode_var = tk.StringVar(value=self.mode)
        mode_menu = ttk.Combobox(mode_frame, textvariable=self.mode_var, width=8, state="readonly")
        mode_menu['values'] = ['十六进制', '二进制']
        mode_menu.pack(side=tk.LEFT, padx=5)
        mode_menu.bind("<<ComboboxSelected>>", self.change_display_mode)
        
        # 编码格式选择
        encoding_frame = ttk.Frame(control_frame)
        encoding_frame.pack(side=tk.LEFT, padx=10)
        
        ttk.Label(encoding_frame, text="编码格式:").pack(side=tk.LEFT)
        
        self.encoding_var = tk.StringVar(value=self.encoding)
        encoding_menu = ttk.Combobox(encoding_frame, textvariable=self.encoding_var, width=8, state="readonly")
        encoding_menu['values'] = ['ascii', 'utf-8', 'latin-1', 'cp1252', 'gbk', 'utf-16']
        encoding_menu.pack(side=tk.LEFT, padx=5)
        encoding_menu.bind("<<ComboboxSelected>>", self.change_encoding)
        
        # 左右分栏
        split_frame = ttk.PanedWindow(editor_container, orient=tk.HORIZONTAL)
        ...
        left_header = ttk.Label(left_frame, text="二进制数据", font=("TkDefaultFont", 10, "bold"))
        left_header.pack(fill=tk.X, padx=5, pady=5)
        
        self.left_text = tk.Text(left_frame, wrap=tk.NONE, width=40, height=25)
        self.left_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # 添加滚动条
        left_scroll = ttk.Scrollbar(left_frame, command=self.on_scroll)
        ...
        # 添加滚动条
        right_scroll = ttk.Scrollbar(right_frame, command=self.on_scroll)
        ...
        self.size_var = tk.StringVar(value="0 字节")
        ...
        self.left_text.bind("<B1-Motion>", self.on_left_drag)
        self.left_text.bind("<ButtonRelease-1>", self.on_left_release)
        self.left_text.bind("<Button-3>", self.show_context_menu)
        self.left_text.bind("<FocusIn>", lambda e: self.update_focus_indicator("二进制数据"))
        
        self.right_text.bind("<Button-1>", self.on_right_click)
        self.right_text.bind("<B1-Motion>", self.on_right_drag)
        self.right_text.bind("<ButtonRelease-1>", self.on_right_release)
        self.right_text.bind("<Button-3>", self.show_context_menu)
        self.right_text.bind("<FocusIn>", lambda e: self.update_focus_indicator("编码值"))
        
        # 配置文本标签
        self.left_text.tag_config("selected", background="red", foreground="white")
        self.left_text.tag_config("signature", background="lightblue")
        
        self.right_text.tag_config("selected", background="red", foreground="white")
        
        # 创建右键菜单
        self.context_menu = tk.Menu(self.parent, tearoff=0)
        self.context_menu.add_command(label="复制选中内容", command=self.copy_selected)
        self.context_menu.add_command(label="选中区域", command=self.select_region)
        self.context_menu.add_command(label="保存选中区域", command=self.save_selected_region)
        self.context_menu.add_command(label="清除选中", command=self.clear_selection)
        
        # 初始化焦点指示器
        self.update_focus_indicator("-")

    def update_focus_indicator(self, widget_name):
        """更新焦点指示器文本"""
        self.focused_widget = widget_name
        self.focus_indicator.config(text=f"当前聚焦: {widget_name}")
    ...
    def open_file(self):
        ...
        if file_path:
            ...
            self.progress_dialog.title("加载文件")
            ...
            ttk.Label(self.progress_dialog, text=f"正在加载: {os.path.basename(file_path)}").pack(pady=10)
            
            self.progress_var = tk.DoubleVar()
            progress_bar = ttk.Progressbar(self.progress_dialog, variable=self.progress_var, 
                                         mode='determinate', length=250)
            progress_bar.pack(pady=5)
            
            self.status_label = ttk.Label(self.progress_dialog, text="0%")
            self.status_label.pack(pady=5)
            
            cancel_button = ttk.Button(self.progress_dialog, text="取消", 
                                      command=self.cancel_file_load)
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                while True:
                    ...
                    self.parent.after(0, lambda p=progress: self.progress_var.set(p))
                    self.parent.after(0, lambda: self.status_label.config(text=f"{progress:.1f}%"))
            
            if not self.cancel_loading:
                self.binary_data = b''.join(chunks)
                self.size_var.set(f"{len(self.binary_data)} 字节")
                self.parent.after(0, self.display_file_content)
                self.save_button.config(state=tk.NORMAL)
        except Exception as e:
            self.parent.after(0, lambda: messagebox.showerror("错误", f"加载文件失败: {str(e)}"))
        finally:
            self.parent.after(0, self.progress_dialog.destroy)
            self.is_loading = False
    ...
    def display_file_content(self):
        ...
        # 显示二进制数据
        if mode == "十六进制":
            hex_data = binascii.hexlify(self.binary_data).decode('ascii')
            # 格式化显示，每行16个字节
            for i in range(0, len(hex_data), 32):
                hex_line = hex_data[i:i+32]
                formatted_line = ' '.join([hex_line[j:j+2] for j in range(0, len(hex_line), 2)])
                self.left_text.insert(tk.END, formatted_line + '\n')
        else:  # 二进制模式
            binary_str = ''.join(f'{byte:08b}' for byte in self.binary_data)
            # 格式化显示，每行16个字节
            for i in range(0, len(binary_str), 128):
                bin_line = binary_str[i:i+128]
                formatted_line = ' '.join([bin_line[j:j+8] for j in range(0, len(bin_line), 8)])
                self.left_text.insert(tk.END, formatted_line + '\n')
        
        # 显示编码值
        try:
            # 尝试用指定编码解码
            text_data = self.binary_data.decode(encoding, errors='replace')
            # 格式化显示，每行16个字符
            for i in range(0, len(text_data), 16):
                text_line = text_data[i:i+16]
                self.right_text.insert(tk.END, text_line + '\n')
        except Exception as e:
            self.right_text.insert(tk.END, f"解码失败: {str(e)}\n")
        
        # 标注文件头文件尾
        self.highlight_signatures()
        
        self.status_var.set(f"文件加载完成: {os.path.basename(self.current_file)}")
    
    def highlight_signatures(self):
        """高亮显示文件签名"""
        for signature, description in self.file_header_footer_dict.items():
            pos = self.binary_data.find(signature)
            if pos != -1:
                # 在左侧文本中高亮签名
                start_index = self.get_text_index(pos, len(signature))
                if start_index:
                    self.left_text.tag_add("signature", start_index, f"{start_index}+{len(signature)*3}c")
                    # 添加工具提示
                    self.left_text.tag_bind("signature", "<Enter>", 
                                          lambda e, d=description: self.show_signature_tooltip(e, d))
                    self.left_text.tag_bind("signature", "<Leave>", self.hide_tooltip)
    
    def get_text_index(self, byte_pos, length=1):
        """根据字节位置获取文本索引"""
        if byte_pos < 0 or byte_pos >= len(self.binary_data):
            return None
        
        # 计算行号和列号
        bytes_per_line = 16
        line = byte_pos // bytes_per_line
        col = byte_pos % bytes_per_line
        
        # 根据显示模式计算位置
        if self.mode_var.get() == "十六进制":
            # 每字节占3个字符 (2个十六进制字符 + 1个空格)
            col_pos = col * 3
        else:  # 二进制模式
            # 每字节占9个字符 (8个二进制字符 + 1个空格)
            col_pos = col * 9
        
        return f"{line+1}.{col_pos}"
    
    def show_signature_tooltip(self, event, description):
        """显示签名工具提示"""
        x, y, _, _ = self.left_text.bbox(tk.CURRENT)
        self.tooltip = tk.Toplevel(self.left_text)
        self.tooltip.wm_overrideredirect(True)
        self.tooltip.wm_geometry(f"+{event.x_root+10}+{event.y_root+10}")
        label = ttk.Label(self.tooltip, text=description, background="#ffffe0", relief="solid", borderwidth=1)
    ...
    def save_file(self):
        """保存文件"""
        if not self.current_file or not self.binary_data:
            return
        
        # 弹出保存选项对话框
        save_option = self.show_save_options_dialog()
        if save_option is None:  # 用户取消了操作
            return
        
        try:
            if save_option == "whole":
                # 保存整个文件
                data_to_save = self.binary_data
            elif save_option == "selected":
                # 保存选中的区域
                if self.selected_start is None or self.selected_end is None:
                    self.status_var.set("没有选中的区域")
                    return
                start = min(self.selected_start, self.selected_end)
                end = max(self.selected_start, self.selected_end) + 1
                data_to_save = self.binary_data[start:end]
            elif save_option == "between":
                # 保存两个标红范围之间的区域
                if self.selected_start is None or self.selected_end is None:
                    self.status_var.set("需要两个标红范围")
                    return
                start = min(self.selected_start, self.selected_end)
                end = max(self.selected_start, self.selected_end) + 1
                data_to_save = self.binary_data[start:end]
            elif save_option == "inverse":
                # 反向保存（保存除选中区域外的内容）
                if self.selected_start is None or self.selected_end is None:
                    self.status_var.set("没有选中的区域")
                    return
                ...
                data_to_save = self.binary_data[:start] + self.binary_data[end:]
            else:
                return
            ...
            self.status_var.set(f"文件保存成功: {os.path.basename(file_path)}")
            self.parent.after(0, lambda: messagebox.showinfo("成功", "文件已保存"))
        
        except Exception as e:
            self.parent.after(0, lambda: messagebox.showerror("错误", f"保存文件失败: {str(e)}"))
    
    def show_save_options_dialog(self):
        """显示保存选项对话框"""
        ...
        ttk.Label(dialog, text="选择保存方式:", font=("TkDefaultFont", 10, "bold")).pack(pady=10)
        
        # 创建选项变量
        save_option = tk.StringVar(value="whole")
        
        # 创建选项按钮
        frame = ttk.Frame(dialog)
        frame.pack(fill=tk.BOTH, padx=20, pady=10)
        
        ttk.Radiobutton(frame, text="保存整个文件", variable=save_option, value="whole").pack(anchor="w", pady=5)
        ttk.Radiobutton(frame, text="仅保存选中的区域", variable=save_option, value="selected").pack(anchor="w", pady=5)
        ttk.Radiobutton(frame, text="保存两个标红范围之间的区域", variable=save_option, value="between").pack(anchor="w", pady=5)
        ttk.Radiobutton(frame, text="反向保存（保存除选中区域外的内容）", variable=save_option, value="inverse").pack(anchor="w", pady=5)
    ...
    def change_encoding(self, event=None):
        ...
        if self.binary_data:
            ...
            self.right_text.delete(1.0, tk.END)
            try:
                text_data = self.binary_data.decode(self.encoding, errors='replace')
                for i in range(0, len(text_data), 16):
                    text_line = text_data[i:i+16]
                    self.right_text.insert(tk.END, text_line + '\n')
            except Exception as e:
    ...
    def on_left_click(self, event):
        ...
        index = self.left_text.index(f"@{event.x},{event.y}")
        ...
        if self.mode == "十六进制":
            byte_pos = (line * 16) + (col // 3)
        else:  # 二进制模式
            byte_pos = (line * 16) + (col // 9)
        
        if byte_pos < len(self.binary_data):
            self.highlight_byte(byte_pos)
            self.selected_start = byte_pos
    ...
    def on_left_drag(self, event):
        ...
        if self.mode == "十六进制":
            ...
            byte极狐_pos = (line * 16) + (col // 9)
        
        if byte_pos < len(self.binary_data):
            self.highlight_range(self.selected_start, byte_pos)
    ...
    def on_right_click(self, event):
        ...
        index = self.right_text.index(f"@{event.x},{event.y}")
        
        # 计算字节位置
        line, col = map(int, index.split('.'))
        line -= 1
        byte_pos = (line * 16) + col
    ...
    def highlight_byte(self, byte_pos):
        """高亮单个字节"""
        # 在左侧文本中高亮
        left_start = self.get_text_index(byte_pos)
        if left_start:
            if self.mode == "十六进制":
                self.left_text.tag_add("selected", left_start, f"{left_start}+2c")
            else:  # 二进制模式
                self.left_text.tag_add("selected", left_start, f"{left_start}+8c")
        
        # 在右侧文本中高亮
        line = byte_pos // 16
        col = byte_pos % 16
        right_start = f"{line+1}.{col}"
        self.right_text.tag_add("selected", right_start, f"{right_start}+1c")
    ...
    def highlight_range(self, start, end):
        ...
        # 在左侧文本中高亮范围
        start_index = self.get_text_index(start)
        end_index = self.get_text_index(end)
        ...
        if start_index and end_index:
            ...
            self.left_text.tag_add("selected", start_index, end_index)
        ...
        if start_line == end_line:
            # 单行选择
            right_start = f"{start_line+1}.{start_col}"
            right_end = f"{end_line+1}.{end_col+1}"
            self.right_text.tag_add("selected", right_start, right_end)
        else:
            # 第一行
            self.right_text.tag_add("selected", f"{start_line+1}.{start_col}", f"{start_line+1}.end")
            
            # 中间行
            for line in range(start_line+1, end_line):
                self.right_text.tag_add("selected", f"{line+1}.0", f"{line+1}.end")
            
            # 最后一行
            self.right_text.tag_add("selected", f"{end_line+1}.0", f"{end_line+1}.{end_col+1}")
    
    def clear_highlights(self):
        """清除所有高亮"""
        self.left_text.tag_remove("highlight", "1.0", tk.END)
        self.left_text.tag_remove("selected", "1.0", tk.END)
        self.right_text.tag_remove("highlight", "1.0", tk.END)
        self.right_text.tag_remove("selected", "1.0", tk.END)
    def highlight_file_signatures(self, binary_data):
        """高亮显示文件签名"""
        for signature, label in self.file_header_footer_dict.items():
            index = binary_data.find(signature)
            if index != -1:
                # 计算行号和列号
                line = index // 16 + 1
                col = index % 16
                start_index = f"{line}.{col}"
                end_index = f"{line}.{col + len(signature)}"
                # 高亮显示文件签名
                self.left_text.tag_add("highlight_purple", start_index, f"{start_index}+{len(signature)*3}c")
                self.left_text.tag_config("highlight_purple", foreground="purple")
                # 在右侧文本中高亮显示文件签名
                right_start = f"{line}.{col}"
                right_end = f"{line}.{col + len(signature)}"
                self.right_text.tag_add("highlight_purple", right_start, f"{right_start}+{len(signature)}c")
                self.right_text.tag_config("highlight_purple", foreground="purple")
    ...
    def copy_selected(self):
        ...
        if focused_widget in [self.left_text, self.right_text]:
            try:
                # 尝试获取选中的文本
                selected_text = focused_widget.get(tk.SEL_FIRST, tk.SEL_LAST)
                self.parent.clipboard_clear()
                self.parent.clipboard_append(selected_text)
                self.status_var.set("已复制选中内容")
            except:
                # 如果没有选中文本，复制当前高亮区域
                if self.selected_start is not None and self.selected_end is not None:
                    start = min(self.selected_start, self.selected_end)
                    end = max(self.selected_start, self.selected_end) + 1
                    selected_data = self.binary_data[start:end]
                    
                    if focused_widget == self.left_text:
                        # 对于左侧，复制十六进制或二进制表示
                        if self.mode == "十六进制":
                            hex_data = binascii.hexlify(selected_data).decode('ascii')
                            self.parent.clipboard_clear()
                            self.parent.clipboard_append(hex_data)
                        else:
                            bin_data = ''.join(format(byte, '08b') for byte in selected_data)
                            self.parent.clipboard_clear()
                            self.parent.clipboard_append(bin_data)
                    else:
                        # 对于右侧，尝试解码文本
                        try:
                            text_data = selected_data.decode(self.encoding, errors='replace')
                            self.parent.clipboard_clear()
                            self.parent.clipboard_append(text_data)
                        except:
                            self.parent.clipboard_clear()
                            self.parent.clipboard_append(selected_data.decode('asci', errors='replace'))
                    
                    self.status_var.set(f"已复制 {end-start} 字节")
    
    def select_region(self):
        """选中区域操作"""
        if self.selected_start is not None and self.selected_end is not None:
            self.highlight_range(self.selected_start, self.selected_end)
    
    def save_selected_region(self):
        """保存选中区域到文件"""
        if self.selected_start is None or self.selected_end is None:
            messagebox.showinfo("提示", "请先选择区域")
            return
        ...
        if file_path:
            try:
                ...
                self.status_var.set(f"已保存 {end-start} 字节到 {os.path.basename(file_path)}")
            except Exception as e:
    ...
    def handle_drop(self, files):
        ...
        if files:
            self.current_file = files[0]
            self.status_var.set(f"正在加载文件: {os.path.basename(self.current_file)}")
            self.is_loading = True
            self.cancel_loading = False
            threading.Thread(target=self.load_file, args=(self.current_file,), daemon=True).start()
    
    def get_text_index(self, byte_pos):
        """根据字节位置获取文本索引"""
        if byte_pos < 0 or byte_pos >= len(self.binary_data):
            return None
    ...
    def select_region(self):
        ...
        try:
        # 检查是否有文本被选中
            if not self.left_text.tag_ranges(tk.SEL):
                messagebox.showinfo("提示", "请先选择要处理的文本")
                return
            
        # 获取选中文本的起始和结束位置
            start_index = self.left_text.index(tk.SEL_FIRST)
            end_index = self.left_text.index(tk.SEL_LAST)
        
        # 获取选中文本
            selected_text = self.left_text.get(start_index, end_index)
        except Exception as e:
            messagebox.showerror("错误", f"选择文本时出错: {str(e)}")
        if self.selected_start is not None and self.selected_end is not None:
            self.highlight_range(self.selected_start, self.selected_end)
    ...
    def save_selected_region(self):
        """保存选中区域到文件"""
        if self.selected_start is None or self.selected_end is None:
            self.parent.after(0, lambda: messagebox.showinfo("提示", "请先选择区域"))
            return
    ...
    def __init__(self, parent):
        ...
        self.file_header_footer_dict = {
            b'\xFF\xD8\xFF': "JPEG文件头",
            b'\x89\x50\x4E\x47\x0D\x0A\x1A\x0A': "PNG文件头",
            b'\x50\x4B\x03\x04': "ZIP文件头",
            b'\x25\x50\x44\x46': "PDF文件头",
            b'\x49\x44\x33': "MP3文件头",
            b'\xFF\xD9': "JPEG文件尾",
    ...
    def init_ui(self, parent_frame):
        ...
        # 配置文本标签
        self.left_text.tag_config("selected", background="red")
        self.left_text.tag_config("highlight", background="blue", foreground="white")
        self.left_text.tag_config("signature", background="#e0e0ff")
        
        self.right_text.tag_config("selected", background="red")
        self.right_text.tag_config("highlight", background="blue", foreground="white")
    ...
    def display_file_content(self):
        ...
        if mode == "十六进制":
            ...
            binary_str = ''.join(format(byte, '08b') for byte in self.binary_data)
            # 格式化显示，每行16个字节
            for i in range(0, len(binary_str), 128):
                bin_line = binary_str[i:i+128]
                formatted_line = ' '.join([bin_line[j:j+8] for j in range(0, len(bin_line), 8)])
    ...
    def highlight_signatures(self):
        ...
        # 添加自定义签名按钮
        self.add_custom_signature_button()
    
    def add_custom_signature_button(self):
        """添加自定义签名按钮"""
        custom_frame = ttk.Frame(self.parent)
        custom_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Button(custom_frame, text="添加自定义签名", 
                  command=self.add_custom_signature).pack(side=tk.RIGHT)
    
    def add_custom_signature(self):
        """添加自定义文件签名"""
        signature = simpledialog.askstring("自定义签名", "输入十六进制签名 (例如: FFD8FF):", parent=self.parent)
        if signature:
            try:
                # 转换十六进制字符串为字节
                signature_bytes = binascii.unhexlify(signature.replace(" ", ""))
                description = simpledialog.askstring("自定义签名", "输入描述:", parent=self.parent)
                if description:
                    self.file_header_footer_dict[signature_bytes] = description
                    self.highlight_signatures()
            except binascii.Error:
                messagebox.showerror("错误", "无效的十六进制签名")
    ...
    def save_file(self):
        ...
        try:
            ...
            self.status_var.set(f"文件保存成功: {os.path.basename(self.current_file)}")
        except Exception as e:
            messagebox.showerror("错误", f"保存文件失败: {str(e)}")
    ...
    def highlight_byte(self, byte_pos):
        ...
        if left_start:
            if self.mode == "十六进制":
                self.left_text.tag_add("highlight", left_start, f"{left_start}+2c")
            else:  # 二进制模式
                self.left_text.tag_add("highlight", left_start, f"{left_start}+8c")
        
        # 在右侧文本中高亮
        line = byte_pos // 16
        col = byte_pos % 16
        right_start = f"{line+1}.{col}"
        self.right_text.tag_add("highlight", right_start, f"{right_start}+1c")
                ...
                if self.selected_start is not None and self.selected_end is not None:
                    ...
                    if focused_widget == self.left_text:
                        ...
                        try:
                            ...
                            self.parent.clipboard_clear()
                            self.parent.clipboard_append(selected_data.decode('ascii', errors='replace'))
    ...
    def __init__(self, parent):
        ...
        self.encoding_var = tk.StringVar(value=self.config["binary_editor"]["encoding"])
        self.reading_file = False  # 新增：文件读取状态标志
        self.cancel_read = False   # 新增：取消读取标志
        self.binary_data = None # 新增：二进制数据缓存
    ...
    def init_ui(self, parent_frame):
        ...
        open_button = ttk.Button(button_frame, text="打开文件", command=self.open_file)
        open_button.pack(side=tk.LEFT, padx=5)
        
        save_button = ttk.Button(button_frame, text="保存文件", command=self.save_file)
        ...
        modes = [("十六进制", "hex"), ("二进制", "binary")]
        for i, (text, value) in enumerate(modes):
            ttk.Radiobutton(mode_frame, text=text, variable=self.mode_var, 
                            value=value, command=self.update_display).pack(side=tk.LEFT, padx=5)
        
        encoding_frame = ttk.Frame(button_frame)
        encoding_frame.pack(side=tk.LEFT, padx=10)
        
        ttk.Label(encoding_frame, text="编码格式:").pack(side=tk.LEFT)
        
        encodings = ["ascii", "utf-8", "utf-16", "cp1252"]
        encoding_menu = ttk.OptionMenu(encoding_frame, self.encoding_var, self.encoding_var.get(), *encodings, command=self.update_display)
        encoding_menu.pack(side=tk.LEFT, padx=5)
        
        # 左右分栏布局
        split_frame = ttk.Frame(editor_container)
        split_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # 左侧十六进制/二进制显示区域
        left_frame = ttk.LabelFrame(split_frame, text="十六进制/二进制")
        left_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 10), pady=10)
        
        scrollbar_left = ttk.Scrollbar(left_frame)
        scrollbar_left.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.left_text = tk.Text(left_frame, wrap=tk.NONE, yscrollcommand=scrollbar_left.set)
        self.left_text.pack(fill=tk.BOTH, expand=True)
        scrollbar_left.config(command=self.left_text.yview)
        
        # 绑定鼠标点击事件
        self.left_text.bind("<Button-1>", self.on_left_click)
        self.left_text.tag_config("highlight", background="blue")
        self.left_text.tag_config("selected", background="red")
        
        # 右侧编码显示区域
        right_frame = ttk.LabelFrame(split_frame, text="编码值")
        right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, pady=10)
        
        scrollbar_right = ttk.Scrollbar(right_frame)
        scrollbar_right.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.right_text = tk.Text(right_frame, wrap=tk.NONE, yscrollcommand=scrollbar_right.set)
        self.right_text.pack(fill=tk.BOTH, expand=True)
        scrollbar_right.config(command=self.right_text.yview)
        
        # 绑定鼠标点击事件
        self.right_text.bind("<Button-1>", self.on_right_click)
        self.right_text.tag_config("highlight", background="blue")
        self.right_text.tag_config("selected", background="red")
        
        # 右键菜单
        self.popup_menu = tk.Menu(self.left_text, tearoff=0)
        self.popup_menu.add_command(label="复制", command=self.copy_selection)
        self.popup_menu.add_command(label="选中区域", command=self.select_region)
        self.left_text.bind("<Button-3>", self.show_popup_menu)
        self.right_text.bind("<Button-3>", self.show_popup_menu)
    ...
    def open_file(self):
        file_path = filedialog.askopenfilename(
            filetypes=[("二进制文件", "*.dat;*.bin;*.hex;*.txt"),("所有文件","*.*")])
        if file_path:
            self.current_file = file_path
            self.progress_dialog = tk.Toplevel(self.parent)
            self.progress_dialog.title("读取文件")
            self.progress_dialog.geometry("300x120")
            self.progress_dialog.resizable(False, False)
            self.progress_dialog.transient(self.parent)
            self.progress_dialog.grab_set()
            
            ttk.Label(self.progress_dialog, text=f"正在读取: {os.path.basename(file_path)}").pack(pady=10)
            ...
            # 添加取消按钮
            cancel_button = ttk.Button(self.progress_dialog, text="取消", 
                                      command=self.cancel_file_read)
            cancel_button.pack(pady=10)
            
            # 在后台线程中读取文件
            self.reading_file = True
            self.cancel_read = False
            threading.Thread(target=self.read_file_with_progress, args=(file_path,), daemon=True).start()
        ...
        try:
            ...
            with open(file_path, 'rb') as f:
                while True:
                    ...
                    self.parent.after(0, lambda p=progress: self.progress_var.set(p))
                    self.parent.after(0, lambda: self.status_label.config(text=f"{progress:.1f}%"))
                    # 移除 self.parent.update_idletasks() 避免UI问题
                    # self.parent.update_idletasks()  # 强制刷新UI
            
            if not self.cancel_read:
                binary_data = b''.join(chunks)
                # 保存二进制数据，然后调用格式化显示方法
                self.binary_data = binary_data
                self.parent.after(0, self.update_display)  # 调用不带参数的格式化方法
        except Exception as e:
            self.parent.after(0, lambda: messagebox.showerror("错误", f"读取文件失败: {str(e)}"))
        finally:
            self.parent.after(0, self.progress_dialog.destroy)
    ...
    def update_display(self):
        ...
        if mode == "hex":
            ...
            for i in range(0, len(binary_str), 64):
                bin_line = binary_str[i:i+64]
                formatted_line = ' '.join([bin_line[j:j+8] for j in range(0, len(bin_line), 8)])
                self.left_text.insert(tk.END, formatted_line + '\n')
        
        try:
            text_data = binary_data.decode(encoding, errors='replace')
            self.right_text.insert(tk.END, text_data)
        except Exception as e:
            self.right_text.insert(tk.END, f"解码失败: {str(e)}")
        
        # 标注文件头文件尾
        self.highlight_file_signatures(binary_data)
    
    def highlight_file_signatures(self, binary_data):
        """高亮显示文件签名"""
        file_header_footer_dict = {
            b'\xFF\xD8\xFF': "JPEG文件头",
            b'\x89\x50\x4E\x47\x0D\x0A\x1A\x0A': "PNG文件头",
            b'\x50\x4B\x03\x04': "ZIP文件头",
            b'\xFF\xD9': "JPEG文件尾"
        ...
        for signature, label in file_header_footer_dict.items():
            index = binary_data.find(signature)
            if index != -1:
                start_index = f"1.0+{index*3}c"
                end_index = f"1.0+{(index + len(signature))*3}c"
                self.left_text.tag_add("highlight_purple", start_index, end_index)
                self.left_text.tag_config("highlight_purple", foreground="purple")
    ...
    def cancel_file_read(self):
        ...
        self.progress_dialog.destroy()
        self.parent.update_status("文件读取已取消")
    
    def save_file(self):
        if not self.current_file:
            self.parent.after(0, lambda: messagebox.showwarning("警告", "请先打开文件"))
            return
        if not hasattr(self, 'binary_data') or self.binary_data is None:
            self.parent.after(0, lambda: messagebox.showwarning("警告", "没有可保存的数据"))
            return
        ...
        try:
            ...
            if mode == "hex":
                # 从文本控件获取十六进制数据
                hex_text = self.left_text.get("1.0", tk.END)
                # 移除空格和换行
                hex_data = hex_text.replace(" ", "").replace("\n", "")
                ...
                # 从文本控件获取二进制数据
                bin_text = self.left_text.get("1.0", tk.END)
                # 移除空格和换行
                bin_data = bin_text.replace(" ", "").replace("\n", "")
                
                # 验证是否为有效二进制
                if not all(c in "01" for c in bin_data):
                    raise ValueError("包含无效的二进制字符")
                
                # 确保长度是8的倍数
                if len(bin_data) % 8 != 0:
                    raise ValueError("二进制数据长度必须是8的倍数")
                
                # 将二进制字符串转换为字节
                binary_data = bytes(
                    int(bin_data[i:i+8], 2)
            ...
            self.parent.update_status(f"文件保存成功: {self.current_file}")
            self.parent.after(0, lambda: messagebox.showinfo("成功", "文件已保存"))
        except Exception as e:
            self.parent.after(0, lambda: messagebox.showerror("错误", f"保存失败: {str(e)}"))
        ...
        if self.current_file:
            ...
            if mode == "hex":
                ...
                for i in range(0, len(hex_data), 2):
                    self.left_text.insert(tk.END, hex_data[i:i+2] + " ")
                    if i % 32 == 30:
                        self.left_text.insert(tk.END, "\n")
            else:  # binary
                binary_str = ''.join(format(byte, '08b') for byte in binary_data)
                for i in range(0, len(binary_str), 8):
                    self.left_text.insert(tk.END, binary_str[i:i+8] + " ")
                    if i % 64 == 63:
                        self.left_text.insert(tk.END, "\n")
            ...
            # 标注文件头文件尾
            for header, label in self.file_header_footer_dict.items():
                index = binary_data.find(header)
                if index != -1:
                    start_index = f"1.0+{index*3}c"
                    end_index = f"1.0+{(index + len(header))*3}c"
                    self.left_text.tag_add("highlight_purple", start_index, end_index)
                    self.left_text.tag_config("highlight_purple", foreground="purple")
    
    def on_left_click(self, event):
        self.left_text.tag_remove("focus", "1.0", tk.END)
        index = self.left_text.index(f"@{event.x},{event.y}")
        self.left_text.tag_add("focus", index, f"{index}+2c")
        self.selected_start = index
        self.selected_end = f"{index}+2c"
    
    def on_left_drag(self, event):
        index = self.left_text.index(f"@{event.x},{event.y}")
        self.left_text.tag_remove("selected", "1.0", tk.END)
        self.left_text.tag_add("selected", self.selected_start, index)
    ...
    def on_right_click(self, event):
        self.right_text.tag_remove("focus", "1.0", tk.END)
        index = self.right_text.index(f"@{event.x},{event.y}")
        self.right_text.tag_add("focus", index, f"{index}+1c")
        self.selected_start = index
    
    def copy_selection(self):
        selected_text = self.left_text.selection_get() or self.right_text.selection_get()
        self.parent.clipboard_clear()
        self.parent.clipboard_append(selected_text)
    ...
    def on_right_drag(self, event):
        index = self.right_text.index(f"@{event.x},{event.y}")
        self.right_text.tag_remove("selected", "1.0", tk.END)
        self.right_text.tag_add("selected", self.selected_start, index)
    ...
    def select_region(self):
        ...
        end_index = self.left_text.index(tk.SEL_LAST)
        self.left_text.tag_add("selected", start_index, end_index)
        self.right_text.tag_add("selected", start_index, end_index)
    ...
    def update_config(self):
        ...
        self.parent.update_status("设置已保存")
